...
class ControllerModuleSmarttabChildNewArrivals extends Controller
{
    public function index($setting)
    {
        ...
        $filter_data = array(
            ...
            'order'       => 'ASC',
            'start'       => 0,
            'limit'       => (isset($setting['limit']) && !empty($setting['limit']) ? $setting['limit'] : 5)
        );

        $results = $this->model_module_smarttab->getLatestProducts(
            $filter_data
        );
